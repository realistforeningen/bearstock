...
class PriceLogicBasic(PriceLogicBase):
    ...
    def _compute_weight(self, code: str, num_lookback_ticks: int=5) -> float:
        ...
        deltaN = self._time_avg(units_sold[:-1]) - units_sold[-1] 
        a = 10
        b = 10
        return a*math.cosh(b*deltaN)

    def _time_avg(self, x):
        foo = sum([math.exp(-i*x[::-1][i]) for i in range(len(x))])
        foo /= max(1, sum([math.exp(-i) for i in range(len(x))]))
        return foo
